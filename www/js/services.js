...
.factory('Wikipedia', function($q, $resource, $location) {
		...
		var favService = {
			...
			clean: function() {
				...
				$cordovaSQLite.execute(this.getDb(), dropTable).then(function(res) {
					...
					console.error("fail drop tb favorite: " + err.code + ": " + err.message);
				});
			},
			prepare: function() {
				...
				$cordovaSQLite.execute(this.getDb(), createTables).then(function(
					...
					console.error("fail prepare tb favorite: " + err.code + ": " + err.message);
				});
				...
				$cordovaSQLite.execute(db, query, []).then(function(res) {
					...
					console.error("fail to check favorite: " + err.code + ": " + err.message);
				});
			},
					...
					.then(function(res) {
						...
						if (isFav > 0 && res.rowsAffected == 0) {
							...
							$cordovaSQLite.execute(db, insert, [name, isFav]).then(
								...
								function(err) {
									console.error("fail to save favorite: " + err.code + ": " + err.message);
									q.reject(err);
								});
						} else {
							q.resolve();
						...
						console.error("fail to recover favorite: " + err.code + ": " + err.message);
						q.reject(err);
			...
			get: function(name) {
				...
				$cordovaSQLite.execute(db, query, [name]).then(function(res) {
					console.log("get favorite " + name + ", number found: " + res.rows.length);
					q.resolve(res.rows.length > 0 ? res.rows.item(0) : null);
				}, function(err) {
					console.error("fail to get favorite: " + err.code + ": " + err.message);
					q.reject(err);
				});
				return q.promise;
			},
			delete: function(name) {
				...
				this.get(parentName).then(function(fav) {
					...
					$cordovaSQLite.execute(db, query, [isFav, name]).then(
						...
						function(err) {
							console.error("fail to delete favorite: " + err.code + ": " + err.message);
							q.reject(err);
						});
				});
		...
		return {
				...
				$cordovaSQLite.execute(db, query, []).then(function(res) {
					if (res.rows.length == 0) {
						...
						$cordovaSQLite.execute(db, initTable, []).then(function(res) {
							...
							console.error("fail init tb category: " + err.code + ": " + err.message);
						});
					}
				}, function(err) {
					console.error("fail to check categories: " + err.code + ": " + err.message);
					return null;
				});
			},
			save: function(name, parentCategoryId) {
				...
				$cordovaSQLite.execute(db, insert, [name, parentCategoryId]).then(
					...
					function(err) {
						console.error("fail to save categories: " + err.code + ": " + err.message);
					});
			},
			get: function(id) {
				var q = $q.defer();
				...
				$cordovaSQLite.execute(db, query, [id]).then(function(res) {
					...
					console.error("fail to get category: " + err.code + ": " + err.message);
					q.reject(err);
				});
				return q.promise;
			},
			list: function() {
				...
				$cordovaSQLite.execute(db, query, []).then(function(res) {
					...
					console.error("fail to get categories: " + err.code + ": " + err.message);
					q.reject(err);
				});
				return q.promise;
			},
	...
	.factory('Settings', function($q, $window, $cordovaSQLite, DB) {
		var db;
		...
		return {
			...
			clean: function() {
				...
				$cordovaSQLite.execute(this.getDb(), dropTable).then(function(res) {
					...
					console.error("fail drop tb settings: " + err.code + ": " + err.message);
				});
			},
			prepare: function() {
				var createTables =
					"CREATE TABLE IF NOT EXISTS settings (_id INTEGER PRIMARY KEY AUTOINCREMENT, _name VARCHAR UNIQUE, _value VARCHAR DEFAULT '');"
				$cordovaSQLite.execute(this.getDb(), createTables).then(function(res) {
					console.log("prepared tb category, rowsAffected: " + res.rowsAffected);
				}, function(err) {
					console.error("fail to prepare tb  categories: " + err.code + ": " +
						err.message);
			...
			save: function(name, value) {
				...
				$cordovaSQLite.execute(db, replace, [name, value]).then(
					...
					function(err) {
						console.error("fail to save settings: " + err.code + ": " + err.message);
					});
			},
			get: function(name) {
				var q = $q.defer();
				...
				$cordovaSQLite.execute(db, query, [name]).then(function(res) {
					...
					console.error("fail to get settings: " + err.code + ": " + err.message);
					q.reject(err);
				});
				return q.promise;
			}
